...
def _single_loss(forward, key, params, seqs, labels):
    out = forward(params, key, seqs)
    yhats = out[..., 0]
    mse = jnp.mean((yhats-labels)**2, axis=0)
    return mse
...
def ensemble_train(
    ...
    opt_init, opt_update = optax.chain(
        ...
        optax.adam(optax.cosine_onecycle_schedule(500, 5e-2))
     )
...
def naive_train(key, forward_t, seqs, labels, val_seqs=None, val_labels=None, params=None, aconfig: AlgConfig = None):
    if aconfig is None:
        aconfig = AlgConfig()
    opt_init, opt_update = optax.chain(
...
def neg_bayesian_ucb(key, f, x, beta=2.):
    joint_out = f(key, x)
    mu = joint_out[0]
    std = jnp.sqrt(joint_out[1])
    ucb = mu + beta * std
