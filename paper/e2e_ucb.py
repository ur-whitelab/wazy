...
#val_seqs = [random.choice(random_seqs) for i in range(1000)]
#val_labels = get_labels(val_seqs)

#with open('result_rapid/val_labels/val_label.pkl', 'wb') as f:
...
for j in range(50):
    ...
    for i in range(50):
        ...
        #for val_seq in val_seqs:
            #yhat_vall.append(forward_t.apply(params, key, get_reps(val_seq)[0]))
        #yhat0.append(yhat[0])
        #yhat1.append(yhat[1])
        #yhats_val.append(yhat_vall)
        yhats.append(yhat)
        y.append(blosum(target_seq, final_vec))
        epi_ales.append(epi_ale)
        params_list.append(params)

    with open('result_ucb/labels/y_{0}.pkl'.format(j), 'wb') as f1:
        pickle.dump(y, f1)
    
    with open('result_ucb/yhats/yhat_{0}.pkl'.format(j), 'wb') as f2:
        pickle.dump(yhats, f2)

    #with open('result_dropout/yhats_val/yhat_val_{0}.pkl'.format(j), 'wb') as f3:
        #pickle.dump(yhats_val, f3)
    
    with open('result_ucb/uncertainty/epi_ale_{0}.pkl'.format(j), 'wb') as f4:
        pickle.dump(epi_ales, f4)

    with open('result_ucb/models/params_{0}.pkl'.format(j), 'wb') as f5:
        pickle.dump(params_list, f5)
