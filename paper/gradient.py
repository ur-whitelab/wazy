...
def loop(key, reps, labels, params, idx):
    ...
    best_v, params, train_loss, grad_loss = alpdesign.grad_iter(
    ...
    return key, reps, labels, s, params, grad_loss, train_loss
...
for j in range(50):
    ...
    for i in range(50):
        ...
        key, reps, labels, final_vec, params, grad_loss, mlp_loss= loop(key, reps, labels, params, i)
        yhat = forward_t.apply(params, key, get_reps([s])[0])
    ...
    with open('grad_result/label/y{0}.pkl'.format(j), 'wb') as f1:
        pickle.dump(y, f1)
    
    with open('grad_result/predict/yhat{0}.pkl'.format(j), 'wb') as f2:
        pickle.dump(yhat0, f2)
    
    with open('grad_result/uncertainty/yhat_{0}.pkl'.format(j), 'wb') as f3:
